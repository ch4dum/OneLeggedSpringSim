...
class Params:
    def __init__(self):
        ...
        self.theta = 3 * (np.pi / 180)  # Fixed angle of 3 degrees
        self.pause = 0.001
        self.fps = 100
...
def animate(z, t, params, wall_height):
    ...
    for i in range(len(t_interp)):
        ...
        x_foot = x - l0 * np.sin(params.theta)
        y_foot = y - l0 * np.cos(params.theta)
...
def start_simulation():
    ...
    torque_input = simpledialog.askfloat("Input", "Enter the applied torque (Nm):", minvalue=0.0)
    ...
    F = torque_input / params.r 

    force_y = F * np.sin(fixed_theta_rad) 
    force_x = F * np.cos(fixed_theta_rad)  
    v_y = np.sqrt((2 * force_y) / params.m)  
    v_x = np.sqrt((2 * force_x) / params.m) 

    params.theta = fixed_theta_rad
    ...
    messagebox.showinfo("Simulation Results",
                        f"Theta: {fixed_theta_deg:.2f} degrees\n"
                        f"Torque: {torque_input:.2f} Nm\n"
...
if __name__ == "__main__":
    ...
    label = tk.Label(root, text="Jump Simulation (Desired Torque)", font=("Helvetica", 12))
    label.pack(pady=10)
