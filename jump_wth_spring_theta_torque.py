...
class Params:
    def __init__(self):
        ...
        self.theta = 3 * (np.pi / 180)  # Fixed angle
        self.pause = 0.001
        self.fps = 100
...
def start_simulation():
    ...
    torque = simpledialog.askfloat("Input", "Enter the desired torque (Nm):", minvalue=0.0)
    if torque is None:
        return
    ...
    if delta_l > params.l:
        messagebox.showerror("Error", "The torque is too large, resulting in an unrealistic spring extension.")
        return
    ...
    force_y = F * np.sin(desired_theta_rad) 
    force_x = F * np.cos(desired_theta_rad) 

    v_y = np.sqrt((2 * force_y) / params.m) 
    v_x = np.sqrt((2 * force_x) / params.m)  
    
    print(f"v_x: {v_x}, v_y: {v_y}, force_x: {force_x}, force_y: {force_y}")
...
if __name__ == "__main__":
    ...
    label = tk.Label(root, text="Jump Simulation (Torque and Theta)", font=("Helvetica", 12))
    label.pack(pady=10)
