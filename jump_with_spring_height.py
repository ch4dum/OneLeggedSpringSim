...
def onestep(z0, t0, params):
    ...
    print(f"Vertical velocity at t={t0:.2f}: {z0[3]:.4f}")
...
def animate(z, t, params, wall_height):
    ...
    for i in range(len(t_interp)):
        ...
        # Print the forward kinematics of the tail
        # print(f"Tail positions at time {t_interp[i]:.3f}:")
        # print(f"  Tail start: (x: {x_tail_start:.4f}, y: {y_tail_start:.4f})")
        # print(f"  Tail end:   (x: {x_tail_end:.4f}, y: {y_tail_end:.4f})")
...
def start_simulation():
    ...
    # เพิ่มความสูงของการกระโดดจริงเป็น 10% มากกว่าความสูงที่กำหนดโดยผู้ใช้
    actual_jump_height = desired_height * 1
    ...
    # คำนวณ y_d ที่จำเป็นในการกระโดดถึงความสูงที่ต้องการจริง
    ...
    # Animation and plotting, including wall with height from user input and position based on max_x
...
if __name__ == "__main__":
    ...
    label = tk.Label(root, text="Jump Simulation (Desired Height)", font=("Helvetica", 12))
    label.pack(pady=10)
